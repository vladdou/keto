...
class RoleApi
{
     ...
     * @param \hades\SDK\Model\GroupMembers $body  (optional)
     ...
     * Client for hades
     *
     * @param \hades\SDK\Model\Role $body  (optional)
     * @throws \hades\SDK\ApiException on non-2xx response
     * @return \hades\SDK\Model\Role
     */
     ...
     * @param \hades\SDK\Model\Role $body  (optional)
     * @throws \hades\SDK\ApiException on non-2xx response
     * @return array of \hades\SDK\Model\Role, HTTP status code, HTTP response headers (array of strings)
     */
    public function createRoleWithHttpInfo($body = null)
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\hades\SDK\Model\Role',
                '/roles'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\hades\SDK\Model\Role', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 201:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\hades\SDK\Model\Role', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
                case 401:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\hades\SDK\Model\InlineResponse401', $e->getResponseHeaders());
